...
import { UploadedImage, GenerationState, SceneTemplate, User } from '../types';
...
const Dashboard: React.FC<DashboardProps> = ({ user, onBuyCredits, onLogout, onCreditsUsed }) => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const result = await generateSelfie(userImage, celebImages, selectedTemplate, customInstructions);
      onCreditsUsed(1);
      setState({ isLoading: false, resultImage: result, error: null });
    } catch (error: any) {
      setState({ 
        isLoading: false, 
        resultImage: null, 
  ...
  const handleDownload = () => {
    ...
    link.download = `selfie-pro-${selectedTemplate.toLowerCase().replace(/\s/g, '-')}-${Date.now()}.png`;
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col items-center">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
              <div className="bg-indigo-50 px-3 py-1.5 rounded-lg flex items-center gap-2">
                  ...
                  <button onClick={onBuyCredits} className="text-xs bg-indigo-600 text-white px-2 py-0.5 rounded hover:bg-indigo-700 transition">
                      + Add
                  </button>
              </div>
      ...
      <main className="flex-1 w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 h-full">
          ...
          {/* Right Column: Result */}
          <div className="flex flex-col items-center justify-start lg:sticky lg:top-24 h-fit">
